...
import { Shield, Calendar, CreditCard, CheckCircle, Users, ArrowRight } from "lucide-react";
...
export const MembershipPage = () => {
    ...
    useEffect(() => {
        // Almacenar la membresía en el estado global cuando se carga
        if (membership) {
        ...
        return (
            <div className="container max-w-6xl mx-auto p-6">
                <Card className="border-destructive/30 bg-destructive/10">
                    <CardHeader>
                        <CardTitle className="text-destructive">No se pudo cargar tu membresía</CardTitle>
                        <CardDescription>
                            {error instanceof Error ? error.message : "Ocurrió un error al cargar los datos de tu membresía"}
                        </CardDescription>
                    </CardHeader>
                    <CardFooter>
    ...
    // Los datos están en el objeto membership directamente, no en membership.activeMembership
    return (
        <div className="container max-w-6xl mx-auto p-6 space-y-8">
            {/* Encabezado */}
            <motion.div
                ...
                <Card className="border shadow-md bg-gradient-to-br from-orange-500/5 to-red-500/5">
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-2xl">Plan {membership.planName}</CardTitle>
                            <Badge variant="outline" className="bg-green-100 text-green-800 border-green-200">
                                <CheckCircle className="mr-1 h-3 w-3" />
                                Activo
                            </Badge>
                        </div>
                        <CardDescription className="mt-2">{membership.daysRemaining} días restantes de membresía</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 rounded-lg bg-gradient-to-br from-orange-500 to-red-600 text-white">
                            <div className="space-y-1 mb-4 sm:mb-0">
                                ...
                                <p className="text-xl font-bold">{formatDate(membership.startDate)}</p>
                            </div>
                            <div className="space-y-1">
                                <h3 className="font-medium">Vigente hasta</h3>
                                <p className="text-xl font-bold">{formatDate(membership.endDate)}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                            <Card className="bg-muted/40">
                                <CardContent className="p-6 flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-medium">{membership.durationMonths} meses</h3>
                            ...
                            <Card className="bg-muted/40">
                                <CardContent className="p-6 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">A todas las instalaciones</p>
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
